...
class VoiceLoader {
    ...
    initialDelay: number;
    maxDelay: number;
    voicesCache: SpeechSynthesisVoice[];
    speechSynthesis: SpeechSynthesis | null;
    loadingPromise: Promise<SpeechSynthesisVoice[]> | null;
     ...
     */
    constructor(config: { maxRetries?: number; initialDelay?: number; maxDelay?: number; speechSynthesis?: SpeechSynthesis | null } = {}) {
        /**
     ...
     */
    async _loadVoicesWithRetry(): Promise<SpeechSynthesisVoice[]> {
     ...
     */
    getRetryConfig(): { maxRetries: number; initialDelay: number; maxDelay: number } {
        return {
